...
function readAgentDetail(name) {
  ...
  // Auto-detect skills from skills/<name>/SKILL.md when not declared in marketplace.json
    ...
    try {
      ...
      const files = entries
        ...
        .flatMap(e => {
          for (const filename of ['SKILL.md', 'README.md']) {
            try {
              const content = stripFrontmatter(fs.readFileSync(path.join(skillsDir, e.name, filename), 'utf8'));
              if (!readmeHtml) readmeHtml = marked(content);
