...
function readAgentDetail(name) {
  ...
  // Skills: path-based (Anthropic spec) or name-based (legacy), with auto-detect fallback
  if (agent.skills) {
    const files = readSkillFiles(agentDir, agent.skills);
    if (files.length > 0) components.push({ label: 'Skills', files });
  } else {
    // Auto-detect from default skills/<name>/SKILL.md directory
    const files = readSkillFiles(agentDir, './skills');
    if (files.length > 0) components.push({ label: 'Skills', files });
  }
