...
function renderAgentCard(agent) {
  ...
  ${typeAttrs}>
  <a class="block h-full no-underline text-inherit" href="/plugins/${encodeURIComponent(agent.name)}">
    ...
    ${agent.author ? `<p class="font-body-sm text-body-sm text-slate-400 mb-2">${escapeHtml(agent.author)}</p>` : ''}
