...
const { renderBadge, getPrimaryMeta, COMPONENT_META } = require('./badge');
...
function renderReferencedFiles(referencedFiles) {
  if (!referencedFiles || referencedFiles.length === 0) return '';
  return `
    <div class="mt-stack-md border-t border-slate-700 pt-stack-md">
      <p class="font-label-caps text-label-caps text-slate-400 uppercase mb-stack-sm flex items-center gap-stack-sm">
        <span class="material-symbols-outlined text-[14px]">description</span>
        Ressources référencées
      </p>
      <div class="space-y-stack-sm">
        ${referencedFiles.map(ref => `
          <details class="group bg-slate-800/30 border border-slate-600 rounded-lg overflow-hidden">
            <summary class="flex justify-between items-center px-stack-md py-stack-sm cursor-pointer hover:bg-slate-700/30 transition-colors list-none">
              <span class="font-code-sm text-code-sm text-slate-300">${escapeHtml(ref.name)}</span>
              <span class="material-symbols-outlined text-slate-400 chevron-icon transition-transform duration-200 text-[16px]">expand_more</span>
            </summary>
            <div class="px-stack-md py-stack-md bg-slate-900/50 border-t border-slate-600">
              <div class="prose-detail">${marked(ref.content)}</div>
            </div>
          </details>`).join('')}
      </div>
    </div>`;
  ...
  return `
    <section class="space-y-stack-md">
      ...
      ${components.map(group => {
        const meta = Object.values(COMPONENT_META).find(m => m.label === group.label) || { icon: 'extension', color: 'slate-400' };
        return group.files.map(file => `
          <details class="group bg-slate-800 border border-slate-700 rounded-xl overflow-hidden" open>
            ...
            <div class="px-gutter py-stack-md bg-slate-900 border-t border-slate-700">
              ...
              ${renderReferencedFiles(file.referencedFiles)}
            </div>
          </details>`).join('');
      }).join('')}
    </section>`;
