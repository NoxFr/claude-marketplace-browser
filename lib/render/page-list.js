...
function renderMarketplaceBanner(marketplaceUrl) {
  ...
  const command = `claude plugin marketplace add ${marketplaceUrl || '<path-or-url>'}`;
  const displayCommand = `claude plugin marketplace add ${source}`;
  return `
    <div class="flex items-center justify-between gap-stack-md px-gutter py-stack-md bg-slate-800/50 border border-slate-700 rounded-xl mb-stack-lg">
      <div class="flex items-center gap-stack-md min-w-0">
        ...
        <div class="min-w-0">
          ...
          <code class="font-code-sm text-code-sm text-on-surface truncate block">${displayCommand}</code>
        </div>
      </div>
      <button
