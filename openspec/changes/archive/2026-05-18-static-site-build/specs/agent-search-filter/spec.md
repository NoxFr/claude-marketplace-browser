...
**Reason**: Remplacé par le filtrage client-side via `data-*` attributes. Plus aucune requête réseau n'est émise lors de la recherche.
**Migration**: Les attributs `hx-get`, `hx-target`, `hx-trigger` sont supprimés des templates. Le module `search.js` gère intégralement le filtrage.
